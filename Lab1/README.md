...
Из последнего шага можно сделать вывод, что **она незавершима** (нетерминируема).
...
Вывод:  **классов эквивалентности конечное число**
...
Получили пять новых правил. Я просчитывала критические пары пока не переставали появляться новые правила. Далее, используя полученный набор правил для редукции слов, большинство слов сводится к небольшому множеству нормальных форм: a, b, c, ab, bb. 
...
Инварианты:
1) Длина слова $∣w_n+1∣≤∣w_n​∣$ (Монотонное невозрастание длины)
2) Ненулевой линейный неотрицательный функционал на векторах счёта букв, который не возрастает I(w)=a+b+2⋅c.


Инварианты которые не подошли:
1) Количество $\{c}$ никогда не увеличивается и может уменьшаться. (нарушается из-за правила ba->c.) Инваринат был проверен в коде.
2) Сумма букв $\{a+c}$ не возрастает. Инваринат был проверен в коде.
3) Максимальная позиция символа $\{c}$ в слове не возрастает. Т.е. символ $\{c}$ не сдвигается в право (нарушается из-за правила cb->bc)
4) Чётность (mod 2) количества X-блоков нечётной длины (заменяем две из трех букв на символы $\{x}$ и считаем длины блоков) Инваринат был проверен в коде.
5) Минимальная позиция символа $\{a}$ не возрастает. Т.е. символ $\{a}$ не сдвигается в право. Инваринат был проверен в коде.
